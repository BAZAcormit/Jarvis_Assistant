...
#Озвучивание голоса помощника
def speak(audio) -> None:
    engine.say(audio)
    engine.runAndWait()

#Получение текущего времени и дальнейшее его форматирование в строку формата "часы:минуты:секунды"
def time() -> None:
...
#Приветствие голосового помощника
def wishme() -> None:
...
#Создание снимка экрана (Скриншота)
def screenshot() -> None:
    img = pyautogui.screenshot()
    img_path = os.path.expanduser("~\\Pictures\\ss.png")
    img.save(img_path)

#Захват голосовой команды с микрофона
def takecommand():
    r = sr.Recognizer()
